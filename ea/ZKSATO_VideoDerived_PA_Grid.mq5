...
input bool   InpUseSessionGate = true;
...
input bool   InpExportTesterStats = true;
...
long     g_margin_mode = 0;
bool     g_hedging_mode = false;
...
int OnInit()
  {
   ...
   if(InpLots<=0.0 || InpGridStepPrice<=0.0 || InpLevelsPerSide<1 ||
      ...
      InpPendingExpiryMinutes<1 || InpMaxSpreadPoints<0 ||
      ...
      (InpUseSessionGate &&
       ...
       (InpSessionStartHour*60+InpSessionStartMinute)))
      return(INIT_PARAMETERS_INCORRECT);

   trade.SetExpertMagicNumber(InpMagic);
   trade.SetAsyncMode(false);
   trade.SetTypeFillingBySymbol(_Symbol);
   g_margin_mode=AccountInfoInteger(ACCOUNT_MARGIN_MODE);
   g_hedging_mode=(g_margin_mode==ACCOUNT_MARGIN_MODE_RETAIL_HEDGING);
   PrintFormat("ZKSATO account mode=%I64d hedging=%s",g_margin_mode,g_hedging_mode ? "true" : "false");
   if(!NettingExposureCompatible())
      return(INIT_FAILED);
...
void OnDeinit(const int reason)
  {
   if(InpExportTesterStats && MQLInfoInteger(MQL_TESTER))
      ExportTesterStatistics();
   PrintFormat("ZKSATO VideoDerived EA deinitialized reason=%d",reason);
  }
...
int VolumeDigits(const double step)
  {
   int digits=0;
   while(digits<8 && MathAbs(step-NormalizeDouble(step,digits))>1e-9)
      digits++;
   return(digits);
  }
...
bool AcceptedTradeRetcode(const uint retcode)
  {
...
bool TradeResultOK(const string action)
  {
   uint retcode=trade.ResultRetcode();
   if(!AcceptedTradeRetcode(retcode))
     {
      PrintFormat("%s rejected retcode=%u description=%s",action,retcode,trade.ResultRetcodeDescription());
      return(false);
     }
   return(true);
  }

bool PendingDistanceOK(const double price,const bool buy_stop)
  {
   MqlTick tick;
   if(!SymbolInfoTick(_Symbol,tick))
      return(false);
...
bool StopLossDistanceOK(const double entry,const double stop_loss,const bool buy_side)
  {
   if(stop_loss<=0.0)
      return(true);
   long stops_level=SymbolInfoInteger(_Symbol,SYMBOL_TRADE_STOPS_LEVEL);
   ...
   double minimum_distance=(double)MathMax(stops_level,freeze_level)*_Point;
   if(buy_side)
      return(stop_loss<=entry-minimum_distance);
   return(stop_loss>=entry+minimum_distance);
  }

bool HasPendingComment(const string comment)
  {
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
     {
      ...
      if(type_time==ORDER_TIME_SPECIFIED && expiration>0 && expiration<=now)
        {
         bool sent=trade.OrderDelete(ticket);
         if(!sent || !TradeResultOK("expired pending delete"))
            PrintFormat("expired pending cleanup failed ticket=%I64u",ticket);
        }
     }
  }

void RebuildStateFromTerminal()
  {
   int positions=CountPositions();
   int pending=CountPendingOrders();
   if(positions>0 || pending>0)
     {
      ...
      PrintFormat("recovered managed cycle after restart positions=%d pending=%d; duplicate seeding suppressed",
                  positions,pending);
     }
  }

bool NettingExposureCompatible()
  {
   if(g_hedging_mode)
      return(true);
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if((ulong)PositionGetInteger(POSITION_MAGIC)!=InpMagic)
        {
         Print("ZKSATO cannot safely manage netting exposure shared with another magic number");
         return(false);
        }
     }
   return(true);
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
     {
      ...
      if(!deleted || !TradeResultOK("delete pending"))
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL)!=_Symbol || (ulong)PositionGetInteger(POSITION_MAGIC)!=InpMagic)
         continue;
      bool closed=(g_hedging_mode ? trade.PositionClose(ticket) : trade.PositionClose(_Symbol));
      if(!closed || !TradeResultOK("close position"))
...
void SeedGrid(const int directional_bias)
  {
   ...
   double max_cycle_volume=NormalizeVolume(InpMaxCycleVolume);
   if(volume<=0.0 || max_cycle_volume<=0.0)
     {
      Print("grid not seeded: requested lots do not align with symbol volume min/max/step");
      return;
     }

   int sides=(directional_bias==0 ? 2 : 1);
   int volume_slots=(int)MathFloor((max_cycle_volume+1e-12)/volume);
   ...
   if(directional_bias>=0)
      PlaceBuyStops(anchor,levels,volume);
   if(directional_bias<=0)
      PlaceSellStops(anchor,levels,volume);
  }

void PlaceBuyStops(const double anchor,const int levels,const double volume)
  {
   ...
   for(int level=1;level<=levels;level++)
     {
      ...
      if(HasPendingComment(comment) || !PendingDistanceOK(price,true) ||
         !StopLossDistanceOK(price,stop_loss,true))
        {
         PrintFormat("BuyStop skipped level=%d price=%.*f: duplicate or stops/freeze constraint",
                     level,_Digits,price);
         continue;
        }
      datetime expiry=TimeCurrent()+(InpPendingExpiryMinutes*60);
      bool sent=trade.BuyStop(volume,price,_Symbol,stop_loss,0.0,
                              ORDER_TIME_SPECIFIED,expiry,comment);
      if(!sent || !TradeResultOK("BuyStop"))
         PrintFormat("BuyStop failed level=%d price=%.*f retcode=%u",level,_Digits,price,trade.ResultRetcode());
     }
  }

void PlaceSellStops(const double anchor,const int levels,const double volume)
  {
   double stop_loss=0.0;
   if(InpGridMode==ZKSATO_PA_FILTERED && g_zone_level>0.0 && g_last_atr>0.0)
      stop_loss=NormalizePrice(g_zone_level+(g_last_atr*InpCycleStopATR));
   ...
   for(int level=1;level<=levels;level++)
     {
      ...
      if(HasPendingComment(comment) || !PendingDistanceOK(price,false) ||
         !StopLossDistanceOK(price,stop_loss,false))
        {
         PrintFormat("SellStop skipped level=%d price=%.*f: duplicate or stops/freeze constraint",
                     level,_Digits,price);
         continue;
        }
      datetime expiry=TimeCurrent()+(InpPendingExpiryMinutes*60);
      bool sent=trade.SellStop(volume,price,_Symbol,stop_loss,0.0,
                               ORDER_TIME_SPECIFIED,expiry,comment);
      if(!sent || !TradeResultOK("SellStop"))
...
void ExportTesterStatistics()
  {
   double profit=TesterStatistics(STAT_PROFIT);
   double equity_drawdown=TesterStatistics(STAT_EQUITY_DD);
   double trades=TesterStatistics(STAT_TRADES);
   double profit_factor=TesterStatistics(STAT_PROFIT_FACTOR);
   PrintFormat("ZKSATO_TESTER_STATS profit=%.2f equity_dd=%.2f trades=%.0f profit_factor=%.4f",
               profit,equity_drawdown,trades,profit_factor);
   int handle=FileOpen("ZKSATO_VideoEA_tester.csv",
                       FILE_WRITE|FILE_CSV|FILE_COMMON,';');
   if(handle==INVALID_HANDLE)
     {
      PrintFormat("tester statistics export failed error=%d",GetLastError());
      return;
     }
   FileWrite(handle,"symbol","magic","profit","equity_dd","trades","profit_factor");
   FileWrite(handle,_Symbol,(string)InpMagic,DoubleToString(profit,2),
             DoubleToString(equity_drawdown,2),DoubleToString(trades,0),
