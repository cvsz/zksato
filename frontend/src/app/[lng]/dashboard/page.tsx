...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchTicker = async () => {
      try {
        const res = await fetch(`${backendUrl}/v1/market/quotes`);
  ...
  useEffect(() => {
    ...
    const fetchPnl = async () => {
      try {
        const res = await fetch(`${backendUrl}/v1/portfolio`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
        if (!cancelled) {
          setPnl({
  ...
  const handleStartBot = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${backendUrl}/v1/bot/start`, {
        method: 'POST',
    ...
    try {
      const adminToken =
        typeof window !== 'undefined'
          ...
          : null;
      const res = await fetch(`${backendUrl}/v1/risk/kill-switch`, {
        method: 'POST',
        headers: adminToken
          ? {
  ...
  const handleRunBacktest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${backendUrl}/v1/backtest`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
