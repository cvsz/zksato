...
The workflow requires an HTTPS zksato endpoint, the literal confirmation `READINESS_ONLY`, externally supplied readiness evidence, and an environment-scoped API credential permitted to call only the zksato readiness evaluation boundary used by operators. Configure required reviewers on the `production` environment and keep the credential outside repository variables, workflow inputs, and committed files.
