...
This document is the operational contract for GitHub Actions, branch protection, dependency maintenance, security automation, releases, UAT evidence, and disaster-recovery drills.
...
| `Release` | `v*` tag | dependency audit, wheel/sdist build, clean install verification, CycloneDX SBOM, checksums, optional attestation, GitHub Release | tag gate |
...
Dependabot keeps `pip` and `github-actions` references current on a weekly Bangkok-time schedule. Action updates are grouped to reduce PR noise while preserving reviewability.
...
Create a protected GitHub environment named `uat` with required reviewers and the environment secret:

- `ZKSATO_UAT_API_KEY` — API key for the deployed UAT control plane. It must never be stored as a repository variable, workflow input, committed file, or PR secret.

The UAT workflow accepts only an HTTPS deployment URL and performs non-mutating certification probes.
...
6. uploads a retained release bundle;
7. optionally generates GitHub artifact attestations;
8. creates the GitHub Release from the tag.

It intentionally does not publish to PyPI or deploy production infrastructure because those require a separately approved distribution/deployment contract.

## UAT, performance, and disaster-recovery evidence
