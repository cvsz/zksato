...
def test_environment_manifest_has_expected_protected_domains() -> None:
    ...
    assert environments["production"]["branch_policies"] == [
...
def test_only_workflow_scoped_application_keys_are_required_in_github() -> None:
    ...
    forbidden = set(manifest["runtime_secret_boundary"]["do_not_install_as_actions_environment_secrets"])
...
def test_environment_url_variables_are_explicit_and_non_secret() -> None:
    ...
    assert environments["production"]["required_variables"] == [
        "ZKSATO_PRODUCTION_BASE_URL"
    ]
