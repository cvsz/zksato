...
class VideoEaConfig(BaseModel):
    ...
    def validate_caps(self) -> VideoEaConfig:
        ...
        if (
            self.lower_price_band is not None
            and self.upper_price_band is not None
            and self.lower_price_band > self.upper_price_band
        ):
            raise ValueError("lower_price_band cannot exceed upper_price_band")
        return self
    ...
    def for_instrument(
        ...
        **overrides: object,
        ...
        if metadata.tick_size is not None:
            values.setdefault("tick_size", metadata.tick_size)
        if metadata.lower_price_band is not None:
...
class VideoDerivedEaPlanner:
    ...
    def plan(self, request: VideoEaPlanRequest) -> VideoEaPlan:
        ...
        if not self._within_price_band(anchor, config):
            return self._empty_plan(
                symbol,
                anchor,
                config,
                "anchor price is outside the trusted instrument price band",
            )
        ...
        if len(triggers) < levels * len(sides):
            reasons.append("ladder truncated by trusted instrument price-band limits")
    ...
    def _triggers(
        ...
        for side in sides:
            ...
            for level in range(1, levels + 1):
                ...
                if not self._within_price_band(price, config):
                    continue
                result.append(
                    VideoEaTrigger(
                        side=side,
    ...
    def _direct_rejection(
        ...
        if lows:
            ...
            if last.low <= level + tolerance and last.close >= level and bullish:
                zone = self._zone("support", level, half_width, index, None, last_index, config)
        ...
        if highs:
            ...
            if last.high >= level - tolerance and last.close <= level and bearish:
                zone = self._zone("resistance", level, half_width, index, None, last_index, config)
    ...
    def _zone(
        ...
        return VideoEaZone(
            ...
            low=max(
                config.tick_size,
                config.lower_price_band or config.tick_size,
                level - half_width,
            ),
            high=min(
                config.upper_price_band or (level + half_width),
                level + half_width,
            ),
    ...
    @staticmethod
    def _within_price_band(price: float, config: VideoEaConfig) -> bool:
        if config.lower_price_band is not None and price < config.lower_price_band:
            return False
        return not (config.upper_price_band is not None and price > config.upper_price_band)
